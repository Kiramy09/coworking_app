...
from core.models import CoworkingSpace, Metropole

# 🧹 Nettoyer les métropoles et leur lien dans CoworkingSpace
CoworkingSpace.objects.update(metropole=None)
Metropole.objects.all().delete()
print("🧹 Métropoles supprimées et liens réinitialisés.")
...
# 🔁 Création des métropoles et attribution
for metro_name, cities in VILLE_METROPOLE.items():
    metropole_obj, _ = Metropole.objects.get_or_create(name=metro_name)
    updated = CoworkingSpace.objects.filter(city__in=cities).update(metropole=metropole_obj)
    print(f"✅ {updated} espaces liés à {metro_name}")
